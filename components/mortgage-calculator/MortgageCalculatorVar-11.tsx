...
const MortgageCalculatorVar11 = ({ is_theme = false, raw_data = {} }: { is_theme?: boolean, raw_data?: any }) => {
        ...
        window.parent.postMessage(
            {
                ...
                data: {
                    ...
                    "type": "section",
                    "component": "MortgageCalculatorVar11",
                    ...raw_data,
                }
            },
    ...
    if (themeSett && themeSett != null) {
        return (
            <section
                className="w-full min-h-[100dvh] py-35 px-4 relative overflow-hidden flex items-center justify-center"
                style={{ background: 'linear-gradient(135deg, #6366F1 0%, #8B5CF6 45%, #EC4899 100%)' }}>
                {/* ambient blurred shapes */}
                <div className="absolute top-[-10%] left-[-10%] w-[420px] h-[420px] rounded-full" style={{ background: 'rgba(255,255,255,0.25)', filter: 'blur(80px)' }} />
                <div className="absolute bottom-[-15%] right-[-10%] w-[460px] h-[460px] rounded-full" style={{ background: 'rgba(255,255,255,0.2)', filter: 'blur(90px)' }} />

                <div className="relative w-full max-w-[680px]">

                    <h1 className="text-4xl font-bold text-center mb-10 text-white drop-shadow-sm">
                        Mortgage Calculator
                    </h1>
                    ...
                    >
                        <div className="space-y-6">
                            ...
                            <label className="flex items-center gap-2.5 cursor-pointer select-none text-white" htmlFor="show_calc">
                                <input type='checkbox' className='w-4 h-4' name='show_calc' id='show_calc' checked={show_calc} onChange={handleShowCalc} />
                                <span className="text-sm">Show calculations and amortization</span>
                            </label>

                            <button
                                onClick={CalculateMortagage}
                                className="w-full py-3.5 rounded-2xl font-semibold flex items-center justify-center gap-2 text-white transition-transform hover:scale-[1.01]"
                    ...
                    >
                        <div className="text-5xl font-bold text-white drop-shadow-sm">{monthly_payment}</div>
                        <div className="text-sm mt-2 text-white/80">estimated monthly payment</div>
                    </div>

                    <div
                        className={`mt-6 rounded-3xl p-6 md:p-8 ${!show_calc ? "hidden" : ""}`}
                        style={{ background: 'rgba(255,255,255,0.16)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.35)' }}
                    >
                        <ol className="list-decimal pl-4 space-y-3 text-sm text-white/90" id="further_brk_dwn"></ol>
                    </div>
...
export default MortgageCalculatorVar11