...
const PropCardVar3 = ({ pro_info, is_theme = false, raw_data = {} }: { pro_info?: any, is_theme?: boolean, raw_data?: any } = {}) => {
    ...
    if (themeSett && themeSett != null && pro_info && pro_info != null) {
        return (
            <div className='relative shadow-xl hover:shadow-2xl rounded-md bg-white border border-gray-200 flex flex-col
                bg-center bg-cover bg-no-repeat h-[500px]' style={{ backgroundImage: `url('${primary_photo}')` }}
                ...
                <div className=' absolute bottom-[15px] w-[93%] left-[3.5%] z-20 bg-white/10 backdrop-blur-sm flex flex-col broder border-gray-200 rounded'>
                    <div className='p-3 flex flex-col z-20'>
                        <div className={`font-semibold text-base flex items-center justify-between`}>
                            <div className={`px-3 py-1 rounded text-gray-100 bg-gray-950/40`}>
                    ...
                    <div className=' w-full bottom-0 grid grid-cols-[repeat(3,1fr)_50px] gap-0.5 *:text-sm
